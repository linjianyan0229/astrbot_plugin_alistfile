...
class AlistClient:
    ...
    async def list_files(self, path: str = "/", page: int = 1, per_page: int = 30) -> Optional[Dict]:
        """获取文件列表"""
        try:
            headers = {}
            if self.token:
    ...
    async def upload_file(self, file_path: str, target_path: str, filename: str = None) -> bool:
        """上传文件到Alist
        ...
        try:
            ...
            async with self.session.put(upload_url, data=file_data, headers=headers) as response:
...
class UserConfigManager:
    ...
    def __init__(self, plugin_name: str, user_id: str):
        ...
        self.config_dir = os.path.join(get_astrbot_data_path(), "plugins_data", plugin_name, "users")
...
class CacheManager:
    ...
    def __init__(self, plugin_name: str):
        self.plugin_name = plugin_name
        # 使用 plugins_data 目录结构
        self.cache_dir = os.path.join(get_astrbot_data_path(), "plugins_data", plugin_name, "cache")
    ...
    def get_cache(self, url: str, path: str, user_id: str, max_age: int = 300) -> Optional[Dict]:
        """获取缓存"""
        try:
...
class GlobalConfigManager:
    ...
    def __init__(self, plugin_name: str):
        # 使用 plugins_data 目录结构
        self.config_dir = os.path.join(get_astrbot_data_path(), "plugins_data", plugin_name)
        os.makedirs(self.config_dir, exist_ok=True)
...
@register("alistfile", "linjianyan", "Alist文件管理插件", "1.0.0", "https://github.com/AstrBotDevs/astrbot_plugin_alistfile")
class AlistFilePlugin(Star):
    ...
    async def initialize(self):
        ...
        if not default_url and not require_auth:
            logger.warning("Alist URL未配置，请使用 /alist config 命令配置或在WebUI中配置")
    ...
    def get_user_config(self, user_id: str) -> Dict:
        ...
        allowed_extensions = self.get_webui_config("allowed_extensions", ".txt,.pdf,.doc,.docx,.zip,.rar,.jpg,.png,.gif,.mp4,.mp3")
        ...
        if require_user_auth:
            ...
            user_config["allowed_extensions"] = allowed_extensions.split(",") if isinstance(allowed_extensions, str) else allowed_extensions
            ...
            return {
                ...
                "allowed_extensions": allowed_extensions.split(",") if isinstance(allowed_extensions, str) else allowed_extensions,
    ...
    def _get_user_upload_state(self, user_id: str) -> Dict:
        ...
        if user_id not in self.user_upload_state:
            self.user_upload_state[user_id] = {
                "waiting": False,
                "target_path": "/"
            }
    ...
    def _format_file_list(self, files: List[Dict], current_path: str, user_config: Dict, user_id: str = None) -> str:
        ...
        # 更新用户导航状态
        if user_id:
            self._update_user_navigation_state(user_id, current_path, all_items[:max_files])
    ...
    async def _download_file(self, event: AstrMessageEvent, file_item: Dict, user_config: Dict):
        ...
        try:
            ...
            async with AlistClient(
                ...
                # 创建临时文件
                downloads_dir = os.path.join(get_astrbot_data_path(), "plugins_data", "alistfile", "downloads")
...
💡 如果服务器不需要登录，只需要设置alist_url即可"""
            ...
            # 验证配置项
            valid_keys = ["alist_url", "username", "password", "token", "max_display_files"]
            ...
            if not self._validate_config(user_config):
                yield event.plain_result("❌ 请先配置Alist URL\n💡 使用 /alist config setup 开始配置向导")
                return
        ...
        if path.isdigit():
            ...
            if item:
                if item.get("is_dir", False):
                    ...
                    # 选择的是文件，启动下载
                    yield event.plain_result(f"📥 正在准备下载文件: {item.get('name', '')}...")
                    async for result in self._download_file(event, item, user_config):
                        yield result
                    return
            else:
                yield event.plain_result(f"❌ 序号 {number} 无效，请使用 /alist ls 查看当前目录")
                return
        ...
        try:
            async with AlistClient(
                ...
                if download_url:
                    ...
                    yield event.plain_result(f"❌ 无法获取下载链接，文件可能不存在或是目录: {target_path}")
        except Exception as e:
...
• /alist ls - 查看当前目录"""
            ...
            yield event.plain_result("❌ 未知操作，支持: /alist upload 或 /alist upload cancel")
